...
- `review_draft` (fix a pasted draft and keep its voice): after `slop-pattern-repair`, hands off to `lint`
- `presentation` (deck, report or company profile copy outside a post): after `strict-human-output`, returns to `written-as-spoken`
